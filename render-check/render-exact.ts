...
const LIMIT_MAG = getArgNum("limit-mag", 20);  // faintest visible apparent magnitude
const SAT_MAG   = getArgNum("sat-mag", 4);     // brighter than this saturates
...
type Star = { x: number; y: number; z: number; lum: number; bprp: number };

// Stream-parse CSV to avoid Node's ~512MB string limit on large result sets.
function fetchStars(): Promise<Star[]> {
  const params = new URLSearchParams({
    x: String(eyeStr[0]), y: String(eyeStr[1]), z: String(eyeStr[2]),
    qx: String(qx), qy: String(qy), qz: String(qz), qw: String(qw),
    near: String(NEAR), far: String(DEPTH),
...
function rasterize(stars: Star[], hdr: Float32Array): void {
  // Map apparent magnitude linearly into perceptual brightness:
  // mag >= LIMIT_MAG → invisible (t <= 0); mag <= SAT_MAG → saturated (t >= 1).
  // This compresses the ~12 orders of magnitude in flux into a viewable range
  // the way the eye and stellar magnitudes do.
  ...
  for (const s of stars) {
    ...
    // Radius grows with perceptual brightness so saturated stars bloom slightly.
